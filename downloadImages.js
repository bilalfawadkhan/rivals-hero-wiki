...
const outputDir = path.join(__dirname, 'public/hero-prestige-images');
...
// Loop through heroes and download images
(async () => {
    for (const hero of heroes) {
        const imageUrl = `https://rivalskins.com/nitropack_static/XzdGixxVOsHOPpHksEwGcgLushIiVnPG/assets/images/optimized/rev-a8762c1/rivalskins.com/wp-content/uploads/marvel-assets/assets/hero-prestige-images/${hero.file_image}.png`;
        const filePath = path.join(outputDir, `${hero.file_image}.png`);
        try {
            await downloadImage(imageUrl, filePath);
            console.log(`Downloaded: ${hero.hero_name}`);
        } catch (err) {
            console.error(`Failed: ${hero.hero_name}`, err.message);
        }
    }
